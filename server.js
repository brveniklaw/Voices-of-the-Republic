...
async function initDB() {
  ...
  const migrations = [
    "ALTER TABLE subscribers ADD COLUMN IF NOT EXISTS trial_started_at TIMESTAMP DEFAULT NOW()",
    "ALTER TABLE subscribers ADD COLUMN IF NOT EXISTS notified_expiry BOOLEAN DEFAULT false",
    "ALTER TABLE subscribers ADD COLUMN IF NOT EXISTS notified_paid BOOLEAN DEFAULT false",
    "ALTER TABLE subscribers ADD COLUMN IF NOT EXISTS stripe_session VARCHAR(255)",
    "ALTER TABLE subscribers ADD COLUMN IF NOT EXISTS stripe_customer VARCHAR(255)",
  ...
  // Create license inquiries table
  await pool.query(`
  ...
  try {
    ...
    res.json({ 
      ...
      env: {
        ...
        PAID_DATE: process.env.PAID_DATE,
      }
    });
  } catch (e) {
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Registration failed. Please try again.", detail: e.message });
  }
});
